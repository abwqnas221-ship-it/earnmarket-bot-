...
# قاعدة بيانات مؤقتة في الذاكرة (User Profile DB)
users_db = {}
...
def get_user_data(user_id):
    ...
    if user_id not in users_db:
        users_db[user_id] = {
            "balance": 35.50,         # رصيد افتتاحي تجريبي
...
def get_main_keyboard():
    ...
    # زر فتح الموقع كاملاً داخل تليجرام (Mini App)
...
def send_welcome(message):
    ...
    welcome_text = (
        ...
        "• تسويق منتجات أكبر الشركات وتلقي العمولات مجاناً بالـ USDT.\n"
        "• سحب الأرباح فوراً بدون رسوم إلى محفظة **MEXC.COM**.\n"
        "• يمكنك الآن فتح المنصة التفاعلية كاملة من داخل تليجرام عبر زِر `🌐 فتح منصة المتجر` أدناه!\n\n"
...
def process_product_link(call):
    ...
    if product:
        affiliate_link = f"https://abwqnas221-ship-it.github.io/earnmarket-app/?usr={call.from_user.id}&prod={product['id']}"
        response_msg = (
            f"✅ **رابط التسويق الخارجي الخاص بك جاهز:**\n\n"
            f"`{affiliate_link}`\n\n"
            f"📌 **المنتج:** {product['title']}\n"
            f"💵 **العمولة عند الشراء:** `${product['commission']} USDT`\n\n"
            f"انسخ الرابط وشاركه في منصات التواصل أو مجموعات تليجرام. كل عملية شراء تُضاف أرباحها فوراً لرصيدك!"
        )
        bot.send_message(call.message.chat.id, response_msg, parse_mode="Markdown")
        bot.answer_callback_query(call.id, "تم استخراج رابطك الفريد بنجاح!")

def show_dashboard(chat_id, user_data):
    dash_text = (
        "📊 **لوحة الأرباح والإحصائيات:**\n\n"
...
def simulate_sale(chat_id, user_data):
    ...
    countries = ['🇸🇦 السعودية', '🇦🇪 الإمارات', '🇶🇦 قطر', '🇰🇼 الكويت', '🇪🇬 مصر', '🇴🇲 عمان']
...
def start_withdrawal_flow(message):
    ...
    if user_data['balance'] < 10.0:
        bot.send_message(
            message.chat.id, 
            f"❌ **عفواً، الحد الأدنى للسحب هو $10.00 USDT.**\nرصيدك الحالي: `${user_data['balance']:.2f} USDT`",
            parse_mode="Markdown"
        )
        return
    ...
    prompt = (
        ...
        f"الرصيد المتاح للسحب: `${user_data['balance']:.2f} USDT`\n\n"
        f"يرجى إرسال **عنوان محفظة USDT (TRC20 أو BEP20)** من حسابك في MEXC الآن:"
    )
...
def process_withdrawal_address(message):
    ...
    if len(address) < 10:
        bot.send_message(message.chat.id, "❌ **عنوان المحفظة غير صحيح.** يرجى إعادة المحاولة من القائمة.")
        return
...
def process_withdrawal_amount(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "❌ **مبلغ غير صحيح.** يرجى كتابة أرقام فقط.")
        return
        
    if amount < 10 or amount > user_data['balance']:
        bot.send_message(message.chat.id, "❌ **المبلغ المطلوب خارج نطاق رصيدك المتاح.**")
        return
    ...
    success_text = (
        f"🎉 **تم إرسال طلب السحب بنجاح إلى MEXC!**\n\n"
...
if __name__ == '__main__':
    print("🤖 Bot EarnMarket Pro is running 24/7...")
